...
def ppo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        ...
        target_kl=0.01, logger_kwargs=dict(), save_freq=10, v_act='relu', h_size=64):
    """
    ...
    # env_name = env_fn.split('.')
    env = gym.make(env_fn)
    # env = gym.make('dm2gym:CheetahRun-v0')
    # import dmc2gym
    # env = dmc2gym.make(domain_name='hopper', task_name='stand', seed=seed)
    env.seed(seed)
    obs_dim = env.observation_space.shape
    act_dim = env.action_space.shape
    env_eval = gym.make(env_fn)

    # env_eval = dmc2gym.make(domain_name='hopper', task_name='stand', seed=seed)
    env_eval.seed(seed)

    v_act = nn.ReLU if v_act == 'relu' else nn.Tanh

    # Create actor-critic module
    ac = actor_critic(env.observation_space, env.action_space, **ac_kwargs, h_size=(h_size, h_size), v_activation=v_act)
