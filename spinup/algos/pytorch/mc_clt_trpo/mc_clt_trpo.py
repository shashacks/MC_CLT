...
import spinup.algos.pytorch.mc_clt_trpo.core as core
...
class GAEBuffer:
    ...
    def store(self, obs, act, rew, logp, d_val, ep_time, u_weight):
        """
        ...
        self.u_weight_buf[self.ptr] = u_weight

        self.ptr += 1

    def finish_path(self, last_val=0, last_u_weight=1):
        """
...
def mc_clt_trpo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
         ...
         save_freq=10, algo='dtrpo', d_output=8, mn_std=20, T=10):
    """
    ...
    def compute_uncertainty_weight(bars):
        av_std = 0.0
        z_values = []
        for i in range(d_output):
        ...
        diff = np.sum(np.abs(bars - normal_bars))
        u_weight = sig(-diff * T) + 0.5
        return u_weight
    ...
    for epoch in range(epochs):
        ...
        for t in range(local_steps_per_epoch):
            ...
            u_weight = compute_uncertainty_weight(d_v_bars)
            u_weights.append(u_weight)

            ep_ret += r
            ep_len += 1
            # u_weight = 1.0
            # save and log
            buf.store(o, a, r, logp, d_v, ep_time, u_weight)
            ...
            if terminal or epoch_ended:
                ...
                if timeout or epoch_ended:
                    ...
                    last_d_v_bars = ac.get_d_v(torch.as_tensor(o, dtype=torch.float32)) 
                    last_u_weight = compute_uncertainty_weight(last_d_v_bars)
                    u_weights.append(last_u_weight)
                else:
                    v = 0
                    last_u_weight = 1
                buf.finish_path(v, last_u_weight)
                if terminal:
        ...
        logger.log_tabular('MeanUncertaintyWeight', np.mean(u_weights))
...
if __name__ == '__main__':
    ...
    mc_clt_trpo(lambda: gym.make(args.env), actor_critic=core.MLPActorCritic,
