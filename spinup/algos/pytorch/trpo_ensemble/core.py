...
class Ensemble_V(nn.Module):
    def __init__(self, num_net, input_dim, hidden_dim, activation):
        super(Ensemble_V, self).__init__()
    ...
    def forward(self, x):
        ...
        return torch.squeeze(torch.stack(outputs).mean(dim=0), -1)

    def forward_with_idx(self, x, i):
        return torch.squeeze(self.models[i](x), -1)

    def forward_std(self, x):
        outputs = []
        for model in self.models:
            outputs.append(model(x))
        return torch.squeeze(torch.stack(outputs).std(dim=0), -1).detach().numpy()
...
class MLPActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space, num_net,
                 hidden_sizes=(64,64), activation=nn.Tanh, h_size=(512, 512), v_activation=nn.ReLU):
        super().__init__()
        ...
        self.ensemble_v  = Ensemble_V(num_net, obs_dim, h_size, v_activation)
